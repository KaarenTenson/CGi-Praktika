...
import java.util.*;
/**I did only 1 controller, because I didn't have to make a lot of routes
 * for bigger project I would make separate controller for fetching pages and fragments
 * **/
@Controller
...
public class FlightController {
    ...
    @Autowired
    public FlightController(DbFlightService flightService) {
    ...
    public String sortFlightTable(Model model, @PathVariable Long id) {
    ...
    public String purchaseFlight(Model model, @PathVariable Long id, @RequestBody Map<String, List<String>> tickets) {
    ...
    public String confirmPurchaseFlight(Model model, @PathVariable Long id, @RequestBody Map<String, List<int[]>> tickets) {
        boolean valid=true;
    ...
    @RequestMapping("find/destination")
    public String findSuitableDestinations(Model model, @RequestParam(required = true) String destination) {
        List<String> destinations = flightService.getAllDestinationsThatContainString(destination);
        model.addAttribute("locations", destinations);
        model.addAttribute("type", "destination");
        return "fragments/availableLocations";
    }
    @RequestMapping("find/departure")
    public String findSuitableDepartures(Model model, @RequestParam(required = true) String departure) {
        List<String> destinations = flightService.getAllDeparturesThatContainString(departure);
        model.addAttribute("locations", destinations);
        model.addAttribute("type", "departure");
        return "fragments/availableLocations";
    }
